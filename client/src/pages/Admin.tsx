...
import { ArrowDown, ArrowLeft, ArrowUp, LogOut, Plus, Save, Shield, Trash2, UserCog } from "lucide-react";
...
export default function Admin() {
  ...
  const save = trpc.admin.content.update.useMutation({ onSuccess: () => { contents.refetch(); setError(""); }, onError: (err) => setError(err.message) });
  const accounts = trpc.admin.accounts.list.useQuery(undefined, { enabled: loggedIn && role === "owner" });
  const createAccount = trpc.admin.accounts.create.useMutation({ onSuccess: () => { setAccountForm({ username: "", password: "", role: "admin" }); accounts.refetch(); setError(""); }, onError: (err) => setError(err.message) });
  const updateAccount = trpc.admin.accounts.update.useMutation({ onSuccess: () => { accounts.refetch(); setError(""); }, onError: (err) => setError(err.message) });
  ...
  const saveDraft = (key: ContentKey = activeKey) => {
    try {
      ...
      if (key === "home" && (!Array.isArray(parsed.screens) || !Array.isArray(parsed.ctas))) throw new Error("홈 홍보에는 screens 배열과 ctas 배열이 필요합니다.");
      if (key === "videos" && (!Array.isArray(parsed.items) || typeof parsed.title !== "string")) throw new Error("영상 목록에는 title 문자열과 items 배열이 필요합니다.");
      if (key === "archive" && (!Array.isArray(parsed.entries) || typeof parsed.intro !== "string")) throw new Error("활동기록소에는 소개 문구와 기록 카드 배열이 필요합니다.");
      if (key === "recruiting" && ["generation", "year", "title", "noticeTitle", "noticeCopy"].some((field) => typeof parsed[field] !== "string")) throw new Error("모집 안내의 기본 문자열 필드를 확인하세요.");
      save.mutate({ contentKey: key, contentValue: JSON.stringify(parsed, null, 2) });
    } catch (validationError) { setError(validationError instanceof Error ? validationError.message : "입력 형식을 확인하세요."); }
  };
