...
export default function Admin() {
  ...
  return <main className="admin-page"><header className="admin-topbar"><div><p className="section-kicker">PULCHERRIMA / ADMIN</p><h1>사이트 콘텐츠</h1></div><div className="admin-top-actions"><Link href="/"><ArrowLeft size={16} /> 사이트 보기</Link><button type="button" onClick={() => logout.mutate()}><LogOut size={16} /> 로그아웃</button></div></header><section className="admin-editor"><aside><button className={activeKey === "archive" ? "is-active" : ""} onClick={() => setActiveKey("archive")} type="button">활동기록소</button><button className={activeKey === "recruiting" ? "is-active" : ""} onClick={() => setActiveKey("recruiting")} type="button">모집 안내</button></aside><div className="admin-editor-main"><div className="admin-editor-heading"><div><p className="section-kicker">편집 중</p><h2>{label}</h2></div><button className="admin-save-button" type="button" disabled={save.isPending} onClick={() => save.mutate({ contentKey: activeKey, contentValue: drafts[activeKey] })}><Save size={16} /> {save.isPending ? "저장 중" : "저장"}</button></div><p className="admin-help">JSON 형식으로 저장합니다. 기본 사이트 구조와 색상은 관리자 편집 대상이 아닙니다.</p><textarea value={drafts[activeKey]} onChange={(event) => setDrafts((current) => ({ ...current, [activeKey]: event.target.value }))} spellCheck={false} aria-label={`${label} JSON 편집`} />{error && <div className="admin-error" role="alert">{error}</div>}</div></section></main>;
}
