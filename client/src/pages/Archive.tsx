...
type ArchiveContent = { intro?: string; label?: string; period?: string; entries?: ArchiveEntry[] };
const fallbackEntries: ArchiveEntry[] = [
  { no: "01", date: "2026. 08", type: "관측 기록", title: "여름 은하수 지도", excerpt: "학교 옥상에서 보이는 별을 하나씩 이어 다음 관측을 위한 지도로 남긴 기록입니다.", image: "https://images.unsplash.com/photo-1444703686981-a3abbc4d4fe3?auto=format&fit=crop&w=1100&q=88", imageAlt: "여름 밤하늘과 은하수" },
  { no: "02", date: "2026. 05", type: "현장 기록", title: "초점을 맞추는 동안", excerpt: "망원경을 처음 조립한 날입니다. 오래 걸려도 같은 대상을 함께 확인한 기록입니다.", image: "https://images.unsplash.com/photo-1419242902214-272b3f66ee7a?auto=format&fit=crop&w=1100&q=88", imageAlt: "별이 있는 밤하늘" },
...
export default function Archive() {
  ...
  const managed = useMemo<ArchiveContent>(() => { try { return managedContent.data ? JSON.parse(managedContent.data.contentValue) as ArchiveContent : {}; } catch { return {}; } }, [managedContent.data]);
  const currentMeta = { ...semesterPages[page], label: page === 0 ? (managed.label ?? semesterPages[page].label) : semesterPages[page].label, period: page === 0 ? (managed.period ?? semesterPages[page].period) : semesterPages[page].period };
  const records = page === 0 && managed.entries?.length ? managed.entries : page === 0 ? fallbackEntries : [];
  return (
    <div className="inner-page archive-page">
      <SiteHeader />
      <main className="inner-main">
        <section className="page-intro archive-intro">
          <div className="page-intro-meta"><span>GBS / 풀체리마</span><span>활동기록소 / {currentMeta.period}</span></div>
